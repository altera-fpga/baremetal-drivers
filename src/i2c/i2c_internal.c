...
size_t i2c_transmit(uintptr_t base, uintptr_t buffer, size_t length) {
    ...
    for (size_t i = 0; i < length; i++) {
        if (i == (uint32_t)0) {
            regs->I2C_DATA_CMD = (uint32_t)(((uint8_t *)buffer)[i]) | I2C_CMD_DATA_RESTART | I2C_CMD_DATA_CMD_WRITE;
        } else if (i != (length - (uint32_t)1)) {
            regs->I2C_DATA_CMD = (uint32_t)(((uint8_t *)buffer)[i]) | I2C_CMD_DATA_CMD_WRITE;
        } else {
            regs->I2C_DATA_CMD = (uint32_t)(((uint8_t *)buffer)[i]) | I2C_CMD_DATA_STOP | I2C_CMD_DATA_CMD_WRITE;
        }
    }
    return length;
}
...
size_t i2c_receive(uintptr_t base, uintptr_t buffer, size_t length) {
    ...
    for (size_t i = 0; i < length; i++) {
        ...
        while (regs->I2C_RXFLR == (uint32_t)0) { // Wait for data to be available
            timeout--;
        ...
        // The lack of an 'else' here is deliberate. The write to I2C_CMD_READ above
        // is needed
        //    to start the read, but then 'length' bytes still need to be read.
        ((uint8_t *)buffer)[i] = (uint8_t)(regs->I2C_DATA_CMD & (uint32_t)0xFF);
    }
    return length;
}
